...
cust.tmpd <-
  "~/r/rdata/download" # define where temporary flies are placed
...
head(effort.data)
write.table(
  effort.data, file = "~/r/rdata/stecf-landings.by.ICES.rectangle", sep = "\t", quote = FALSE, row.names = FALSE
)
...
write.table(
  econ.data, file = "~/r/rdata/stecf-economic-fleet.data", sep = "\t", quote = FALSE, row.names = FALSE
)
unlink(econ.data)
